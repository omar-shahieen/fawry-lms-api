...
class CourseGradesIntegrationTest {
    ...
    @Test
    void courseGradesArePagedAndLimitedToOwnerUnlessAdmin() throws Exception {
        ...
        User thirdStudent = saveUser(Role.STUDENT, "Third Student");
        Course ownedCourse = saveCourse(owner, "Owned Course");
        Course otherCourse = saveCourse(otherInstructor, "Other Course");
        Quiz ownedQuiz = saveQuiz(ownedCourse);
        Quiz otherQuiz = saveQuiz(otherCourse);
        saveAttempt(ownedQuiz, firstStudent, 1);
        saveAttempt(ownedQuiz, secondStudent, 2);
        saveAttempt(otherQuiz, thirdStudent, 3);

        mockMvc.perform(get("/api/courses/{courseId}/grades", ownedCourse.getId())
                ...
                .andExpect(jsonPath("$.content[0].student.id").isNotEmpty())
                .andExpect(jsonPath("$.content[0].quiz.id").value(ownedQuiz.getId().intValue()))
                .andExpect(jsonPath("$.content[0].score").isNotEmpty());

        mockMvc.perform(get("/api/courses/{courseId}/grades", otherCourse.getId())
                        .header("Authorization", bearerToken(owner)))
        ...
        mockMvc.perform(get("/api/courses/{courseId}/grades", otherCourse.getId())
                ...
                .andExpect(jsonPath("$.content[0].student.id").value(thirdStudent.getId().toString()))
                .andExpect(jsonPath("$.content[0].quiz.id").value(otherQuiz.getId().intValue()));
    }
